...
import mas.simulator.env.{MeasureBuilder, WideMeasure, Message, OuterWorld}
...
trait DumbEnvironments {
  ...
  class EnvironmentImpl(val linearSize: Int, val receiverLimit: Int,
    ...
    private val surface = new mutable.HashMap[Point, Int]()
    ...
    protected def processMessage(message: Message[_]) {
    ...
    protected def getAllMessagesFor(id: Int): Iterable[Message[_]] = {
      agents find (_.agent.getId == id) map {
        case a =>
          val (x, y) = a.pos
          val (setX, setY) = if (!flag) (messagesX1, messagesY1) else (messagesX2, messagesY2)
    ...
    protected def measureFor(point: (Int, Int)): Int = surface get point getOrElse {
    ...
    private class EnvPartImpl(myAgent: AgentWithPos, agents: Seq[AgentInfo]) extends EnvPart {
      ...
      def moveLeft() {
        env move (id, EnvironmentImpl.left)
      }

      def moveRight() {
        env move (id, EnvironmentImpl.right)
      }

      def moveUp() {
        env move (id, EnvironmentImpl.up)
      }

      def moveDown() {
        env move (id, EnvironmentImpl.down)
      }
